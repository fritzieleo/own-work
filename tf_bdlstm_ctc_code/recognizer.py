...
def detect():
    model_restore = "models/ocr.model-0.805224116751-32999" #give the path of your final model file
    test_filename = "test.txt" #txt file containing the paths of all the test images
    output_folder = "test_outputs/" #where the outputs will be stored
    factor1 = 10 #think of it as number of test_batches
    factor2 = 226 #think of it as the batch size
    ...
    with tf.Session() as sess:
        ...
        for x in range(0,factor1):
    		...
    		for x,fname_save in enumerate(names_list):
    		    ...
    		    file = codecs.open(output_folder + str(lp[0])+os.path.basename(fname_save) + ".out.txt","w", "utf-8")
